...
float *read_float(char *line, int n)
{
...
float* read_from_base64(char* line, float* out) {
    ...
    int base_len = ceil(line_size / 3.0) * 4;
    unsigned char* base_out ;
    if(out == NULL)
        base_out = calloc(line_size, 1);
    else
        base_out = (unsigned char*)out;
    base64_decode(line, base_len, base_out);
    return (float*)base_out;
}
